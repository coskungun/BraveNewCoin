...
protocol MarketListViewModelDelegate {
    func handleViewModelOutput(_ output: MarketListModalViewModelOutput)
}

enum MarketListModalViewModelOutput: Equatable {
    case updateTitle(String)
    case reloadData
    case isErrorConnection(String)
}

class MarketListViewModel:MarketListViewModelProtocol
{
    var delegate: MarketListViewModelDelegate?
    var arrayList:[ArrContentList] = Array.init()
    ...
    func load(view: UIView) {
        notify(.updateTitle("Market List"))
    ...
    private func notify(_ output: MarketListModalViewModelOutput) {
    ...
    func getData()
    {
        ...
        networkHelper?.getAllMarketData(complation: {
            result in
            switch result
            {
